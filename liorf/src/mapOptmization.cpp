...
public:
    ...
    bool saveMapService(liorf::save_mapRequest& req, liorf::save_mapResponse& res)
    {
      ...
      saveOptimizedVerticesKITTIformat(isamCurrentEstimate, saveMapDirectory + "/keyframe_poses.txt");
        ...
        if (cloudInfo.odomAvailable == true)
        {
            ...
            if (lastImuPreTransAvailable == false)
            {
                ...
                if (is_valid_preintegration) {
                    ...
                    lastImuPreTransformation = transTobe;
                    lastImuTransformation = pcl::getTransformation(0, 0, 0, cloudInfo.imuRollInit, cloudInfo.imuPitchInit, cloudInfo.imuYawInit); // save imu before return;
                }
        ...
        for (int i = 0; i < laserCloudSurfLastDSNum; i++)
        {
            ...
            if (pointSearchSqDis[4] < 1.0) {
                ...
                if (planeValid) {
                    ...
                    float effective_dist = std::max(dist, 0.3f);
                    float s = 1.0f - 0.9f * fabs(pd2) / sqrt(effective_dist);
